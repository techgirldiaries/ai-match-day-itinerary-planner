            ...
            <div class="flex flex-col gap-1">
              ...
              <input
                ...
                onInput={(e) => {
                  ...
                  // Allow empty state while typing, only parse when there's content
                  if (val === "") {
                    updateField("groupSize", 1); // Reset to minimum on empty
                  } else {
                    const parsed = parseInt(val, 10);
                    if (!isNaN(parsed)) {
                      updateField("groupSize", parsed);
                    }
                  }
                }}
                onBlur={(e) => {
          ...
          <div class="flex flex-col gap-1">
            ...
            <input
              ...
              onInput={(e) => {
                ...
                // Allow empty state while typing, only parse when there's content
                if (val === "") {
                  updateField("budgetPerPerson", 20); // Reset to default on empty
                } else {
                  const parsed = parseFloat(val);
                  if (!isNaN(parsed)) {
                    updateField("budgetPerPerson", parsed);
                  }
                }
              }}
              onBlur={(e) => {
                ...
                if (val === "" || isNaN(parseFloat(val))) {
                  updateField("budgetPerPerson", 20);
                }
              }}
              class="bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 
                    rounded-lg px-3 py-2 text-gray-900 dark:text-white text-sm
